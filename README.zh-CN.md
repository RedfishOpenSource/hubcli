...
对于这两种 Windows Release 产物，大多数命令都不需要本机额外安装 Node.js 或 Python。
...
这种安装方式仍然要求本机 `PATH` 上存在 Python `3.11+`，并且当前 Python 环境中已安装 `python/pyproject.toml` 声明的依赖。
...
在一台具备 PowerShell、Python `3.11+` 且 `PATH` 上可用 Inno Setup `iscc` 的 Windows 构建机上执行：
...
Release 构建会一并打包 Node.js runtime、PyInstaller 生成的 Python Worker、Playwright Chromium，以及 Arthas 资源。
...
打包结果会输出到项目根目录，例如 `hubcli-<version>.tgz`。
